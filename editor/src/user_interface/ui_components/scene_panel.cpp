	...
	void ScenePanel::Draw(bool* is_open)
	{
		if (*is_open)
		{
			ImGui::Begin("Scene panel");
			{
				...
				{
					if (i == SelectedSkyboxIDFirst)
					{
                        SelectedSkyboxNameFirst = path;
					}
					else if (i == SelectedSkyboxIDSecond)
					{
						SelectedSkyboxNameFirst = path;
					}
				...
				ImGui::BeginChild("Active skyboxes", ImVec2(0, window_size.y * 0.2f), true);
				{
					...
					if (ImGui::Combo("First skybox", &SelectedSkyboxIDFirst, names.data(), Loader::sLoadedSkyboxes->size()))
					{
						Renderer::sSkyboxFirst = Loader::sLoadedSkyboxes->at(SelectedSkyboxNameFirst);
					}

					if (ImGui::Combo("Second skybox", &SelectedSkyboxIDSecond, names.data(), Loader::sLoadedSkyboxes->size()))
					{
